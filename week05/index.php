<?php
require("../../course.php");
$dir = "labs/week05"; $zip = "$dir/lab.zip"; $code = "$dir/code";
echo startPage("Week 05 Lab Exercise","","");

const DEADLINE    = "Sun 24 Mar 2019, 23:59:59";
const GIVE_ASSIGN = "lab05";
const GIVE_FILES  = ["Map.c"];
const OBJECTIVES  =
...
const PROVIDED_FILES =
    ...
    , "Places.c" => "implementation of Places ADT"
    , "Map.h" => "interface to Map/graph ADT"
    , "Map.c" => "almost complete implementation of Map ADT"
    , "pl.c" => "main program to check the Places ADT (<tt>nameToID</tt>, <tt>abbrevToID</tt>)"
    , "euro.c" => "main program to test the Map ADT (<tt>addLink</tt>)"
    , "conn.c" => "main program to test <em>your</em> mods to the Map ADT (<tt>connections</tt>)"
    , "euro.txt" => "expected output from the <tt>euro</tt> program"
    , "links.txt" => "a text representation of the edges in the Map/graph"
    ];
?>
...
<?php echo show_objectives(OBJECTIVES) ?>
<h2>Admin</h2>
...
<dt>Submit</dt><dd><code><?php echo show_give_command(GIVE_ASSIGN, GIVE_FILES)?></code> or via WebCMS</dd>
<dt>Deadline</dt><dd>submit by <?php echo DEADLINE?></dd>
</dl>

<p>
Do this lab in pairs with your new lab partner ("Lab Pairs Weeks 5-10" group).
Please put your names in a comment at the top of each file that you submit.
...
<center><img src="Pics/map.png"></center>
...
<pre><kbd is="sh">unzip <?php echo courseFile($zip)?></kbd></pre>
<p>
...
<?php echo list_files(PROVIDED_FILES) ?>
...
your tutor.
Ideally, you would do this in the Week 05 Lab. If you can't finish the tasks
in the lab, you have until midnight on Sunday to complete and submit them.
You should then demonstrate your work at the start of the Week 06 Lab.
